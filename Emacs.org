...
#+end_src
** Eglot
Eglot calls Flymake by default. Taken from [[https://gluer.org/blog/eglot-keybindings-that-i-use/][gluer]].
#+begin_src emacs-lisp
(use-package eglot
  :ensure nil
  :custom
...
#+end_src
** Ebuild
You must install it with Portage
#+begin_src emacs-lisp
(use-package ebuild-mode
...
*Educational*! We must use require in order to make sure that the packages are loaded correctly before using setq.
